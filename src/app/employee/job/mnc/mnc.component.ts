...
export class MncComponent {
  ...
  search() {
    this.filteredJobs = this.jobs.filter((job) => {
      ...
      const skillCondition =
        ...
        this.selectedSkills.some((skill: string) =>
          job.title.toLowerCase().includes(skill)
        );

      const locationCheckboxCondition =
        !this.selectedLocations.length ||
        this.selectedLocations.some((location: string) =>
  ...
  checkSalary(salary: string): boolean {
    ...
    return (
      salaryRange[0] >= this.salaryLeftValue &&
      salaryRange[1] <= this.salaryRightValue
    );
  }
  ...
  checkExperience(experience: string): boolean {
    ...
    return (
      experienceRange[0] >= this.leftValue &&
      experienceRange[1] <= this.rightValue
    );
  }
  ...
  filterJobs() {
    this.filteredJobs = this.jobs.filter((job) => {
      const experienceRange = job.experience
        .split('-')
        .map((value) => parseInt(value.trim()));

      return (
        experienceRange[0] >= this['selectedRange'][0] &&
        experienceRange[1] <= this['selectedRange'][1]
      );
    });
  }

  isUserLoggedIn: boolean = false;
  showSaveAndApplyButtons: boolean = false;
  ...
  ngOnInit() {
    this.authService['isLoggedIn$'].subscribe((loggedIn: boolean) => {
      ...
      this.showSaveAndApplyButtons = loggedIn;
    });
  }
  save() {}
  apply() {}
